...
def run_episode(env, controller, model, model_type, target_xyz, max_steps=150, tolerance=0.02, render=False, viewer=None, window_size=1):
    """
...
def run_evaluation(args, test_episodes):
        ...
        if model_type != "mpc" and args.num_workers and args.num_workers > 1:
            ...
            with mp.Pool(processes=args.num_workers) as pool:
                for metrics in pool.imap_unordered(_run_episode_worker, worker_args):
                    all_metrics.append(metrics)
                    print(
                        f"  Episode {metrics['episode'] + 1}/{total_episodes}: Steps={metrics['num_steps']}, "
    ...
    if len(all_metrics) > 0:
        ...
        print(
            f"Success Rate @ 0.03m (moderate): {success_rate_relaxed * 100:.2f}% ({int(success_rate_relaxed * len(all_metrics))}/{len(all_metrics)} episodes)"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Closed-loop evaluation using dataset episodes")
    ...
    parser.add_argument("--num-episodes", type=int, default=1000, help="Number of test episodes to generate (default: 1000)")
    parser.add_argument(
        "--num-workers",
        type=int,
        default=CPU_CORES,
    ...
    parser.add_argument("--max-steps", type=int, default=None, help="Max steps per episode (default: None, uses full episode length)")
    parser.add_argument(
        "--tolerance",
        type=float,
        default=0.02,
    ...
    # Visualization
    parser.add_argument("--render", action="store_true", default=True, help="Render episodes in MuJoCo viewer (default: True)")
    parser.add_argument("--no-render", action="store_false", dest="render", help="Disable MuJoCo viewer rendering")

    # Output
    parser.add_argument("--output-dir", type=str, default="results/closed_loop", help="Directory to save results (default: results/closed_loop)")

    # Random seed
    parser.add_argument("--seed", type=int, default=42, help="Random seed for episode selection (default: 42)")

    # Utility
    parser.add_argument("--list-models", action="store_true", help="List all available models in results/scikit_learn_baseline/models/ and exit")
    parser.add_argument("--single-model", action="store_true", help="Evaluate only a single model (use with --model-path or --controller-type)")
    ...
    if len(sys.argv) == 1:  # Only script name, no arguments
        while True:  # Main menu loop
            ...
            if choice == "1":
                ...
                env_temp = MuJoCoEnvironment("models/3dof_robot_arm.xml")
                test_episodes = [sample_3d_cartesian_target(env_temp) for _ in range(args.num_episodes)]
                print("Episodes generated. All models will be evaluated on identical episodes.\n")
                ...
                env_temp = MuJoCoEnvironment("models/3dof_robot_arm.xml")
                test_episodes = [sample_3d_cartesian_target(env_temp) for _ in range(args.num_episodes)]
                print("Episodes generated. All controllers will be evaluated on identical episodes.\n")
    ...
    if args.list_models:
        ...
        if available:
            ...
            print("\nUsage examples:")
            print("  python scripts/closed_loop_eval.py  # Evaluates all models by default")
    ...
    if args.controller_type is None and args.model_path is None:
        ...
        if args.controller_type == "mpc":
            ...
            print("Using MPC controller from src/mpc_surrogate/mpc_controller.py")
