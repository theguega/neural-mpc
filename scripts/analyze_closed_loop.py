...
metric_cols = [
    ...
    "CPU Time (s)",
    "CPU Percent",
    "Steps to Success @0.02m",
    "Steps to Success @0.03m",
    "Steps to Success @0.05m",
    "Steps (All Episodes)",
    "Tracking Error (m)",
    "Control Effort",
    "Mean Joint Error (MPC only)",
    "Action Diff from MPC",
    "Wall Time (s)",
]
...
def color_map(types):
    palette = {"pytorch": "#2ecc71", "sklearn": "#3498db", "mpc": "#e67e22"}
    return [palette.get(t, "#7f8c8d") for t in types]


legend_elements = [Patch(facecolor="#2ecc71", label="PyTorch"), Patch(facecolor="#3498db", label="Scikit-Learn"), Patch(facecolor="#e67e22", label="MPC")]
